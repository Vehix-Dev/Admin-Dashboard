...
"[project]/node_modules/@otplib/core/dist/index.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var s = class extends Error {
    ...
    constructor(e, t){
        super(e, t), this.name = "Base32Error";
    }
}, C = class extends B {
    constructor(e, t){
        super(`Base32 encoding failed: ${e}`, t), this.name = "Base32EncodeError";
    }
}, S = class extends B {
    constructor(e, t){
        super(`Base32 decoding failed: ${e}`, t), this.name = "Base32DecodeError";
    }
}, G = class extends s {
    constructor(e){
    ...
    constructor(e, t){
        super(`Counter tolerance size must not exceed ${e}, got ${t}. Large tolerances can cause performance issues.`), this.name = "CounterToleranceTooLargeError";
    }
}, R = class extends s {
    constructor(e){
...
var ar = new TextEncoder, F = 16, V = 64, J = 20, Q = 1, Z = 3600, rr = 30, er = Number.MAX_SAFE_INTEGER, tr = 100, K = Symbol("otplib.guardrails.override"), c = Object.freeze({
    MIN_SECRET_BYTES: F,
    MAX_SECRET_BYTES: V,
    MIN_PERIOD: Q,
    MAX_PERIOD: Z,
...
function pr(r, e = c) {
    if (r.length < e.MIN_SECRET_BYTES) throw new l(e.MIN_SECRET_BYTES, r.length);
    if (r.length > e.MAX_SECRET_BYTES) throw new g(e.MAX_SECRET_BYTES, r.length);
}
function lr(r, e = c) {
...
function dr(r, e = c) {
    if (!Number.isInteger(r) || r < e.MIN_PERIOD) throw new y(e.MIN_PERIOD);
    if (r > e.MAX_PERIOD) throw new f(e.MAX_PERIOD);
}
function Er(r, e) {
    if (r.length !== e) throw new P(e, r.length);
    if (!/^\d+$/.test(r)) throw new O;
}
function Tr(r, e = c) {
    if ((Array.isArray(r) ? r.length : r * 2 + 1) > e.MAX_WINDOW * 2 + 1) throw new w(e.MAX_WINDOW, Array.isArray(r) ? r.length : r);
}
function mr(r, e = rr, t = c) {
    let [o, n] = Array.isArray(r) ? r : [
        r,
        r
    ];
    if (o < 0 || n < 0) throw new _;
    let i = t.MAX_WINDOW * e, j = Math.max(o, n);
    if (j > i) throw new M(i, j);
}
function xr(r) {
...
function z(r) {
    return typeof r == "string" ? ar.encode(r) : r;
}
function Or(r) {
...
function br(r, e) {
    return typeof r == "string" ? (H(e), e.decode(r)) : r;
}
function Ar(r) {
    let { crypto: e, base32: t, length: o = J } = r;
    or(e), H(t);
...
function _r(r) {
    if (typeof r != "string") throw new k;
}
function sr(r) {
    return {
        ok: !0,
...
var X = class {
    ...
    hmacSync(e, t, o) {
        try {
            let n = this.crypto.hmac(e, t, o);
            if (n instanceof Promise) throw new a("Crypto plugin does not support synchronous HMAC operations");
            return n;
        } catch (n) {
...
function Dr(r) {
    return r.crypto !== void 0 && r.base32 !== void 0;
}
function Nr(r) {
    return r.crypto !== void 0;
}
function qr(r) {
    return r.base32 !== void 0;
}
;
 //# sourceMappingURL=index.js.map
}),
"[project]/node_modules/@otplib/uri/dist/index.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var l = class extends Error {
    ...
    constructor(e, n){
        super(`Invalid value for parameter '${e}': ${n}`), this.name = "InvalidParameterError";
    }
};
var m = 2048, d = 512, y = 1024;
function O(r, e) {
    let n = r instanceof Error ? r.message : String(r);
    return `Invalid URI encoding in ${e}: ${n}`;
}
function u(r, e, n) {
    if (r.length > e * 3) throw new a(`${n} exceeds maximum length`);
    try {
        let t = decodeURIComponent(r);
        if (t.length > e) throw new a(`${n} exceeds maximum length of ${e} characters`);
        return t;
    } catch (t) {
        throw t instanceof a ? t : new a(O(t, n));
    }
}
function f(r) {
    if (r.length > m) throw new a(`URI exceeds maximum length of ${m} characters`);
    if (!r.startsWith("otpauth://")) throw new a(r);
    let e = r.slice(10), n = e.indexOf("/");
    if (n === -1) throw new a(r);
    let t = e.slice(0, n);
    if (t !== "hotp" && t !== "totp") throw new c("type", t);
    let i = e.slice(n + 1), o = i.indexOf("?"), s, p;
    o === -1 ? (s = u(i, d, "label"), p = "") : (s = u(i.slice(0, o), d, "label"), p = i.slice(o + 1));
    let I = P(p);
    return {
        type: t,
        label: s,
...
function P(r) {
    ...
    for (let t of n){
        let i = t.indexOf("=");
        if (i === -1) continue;
        let o = u(t.slice(0, i), 64, "parameter key"), s = u(t.slice(i + 1), y, `parameter '${o}'`);
        switch(o){
            case "secret":
                e.secret = s;
                break;
            ...
            case "digits":
                ...
                break;
            case "counter":
                e.counter = parseInt(s, 10);
                break;
            case "period":
                e.period = parseInt(s, 10);
                break;
        }
    }
    return e;
...
function T(r) {
    ...
    throw new c("algorithm", r);
}
function b(r) {
    let e = parseInt(r, 10);
    if (e === 6 || e === 7 || e === 8) return e;
    throw new c("digits", r);
}
function h(r) {
    let { type: e, label: n, params: t } = r, i = n.split(":").map((p)=>encodeURIComponent(p)).join(":"), o = `otpauth://${e}/${i}?`, s = [];
    return t.secret && s.push(`secret=${t.secret}`), t.issuer && s.push(`issuer=${encodeURIComponent(t.issuer)}`), t.algorithm && t.algorithm !== "sha1" && s.push(`algorithm=${t.algorithm.toUpperCase()}`), t.digits && t.digits !== 6 && s.push(`digits=${t.digits}`), e === "hotp" && t.counter !== void 0 && s.push(`counter=${t.counter}`), e === "totp" && t.period !== void 0 && t.period !== 30 && s.push(`period=${t.period}`), o += s.join("&"), o;
}
function x(r) {
    let { issuer: e, label: n, secret: t, algorithm: i = "sha1", digits: o = 6, period: s = 30 } = r, p = e ? `${e}:${n}` : n;
    return h({
        type: "totp",
        label: p,
        params: {
...
function U(r) {
    let { issuer: e, label: n, secret: t, counter: i = 0, algorithm: o = "sha1", digits: s = 6 } = r, p = e ? `${e}:${n}` : n;
    return h({
        type: "hotp",
        label: p,
        params: {
...
"[project]/node_modules/@otplib/hotp/dist/index.js [app-route] (ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@otplib/core/dist/index.js [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$uri$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@otplib/uri/dist/index.js [app-route] (ecmascript)");
;
;
;
var m = class {
    ...
    guardrails;
    constructor(t = {}){
        this.options = t, this.guardrails = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createGuardrails"])(t.guardrails);
    }
    generateSecret() {
        let { crypto: t, base32: e } = this.options;
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(t), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireBase32Plugin"])(e), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateSecret"])({
            crypto: t,
            base32: e
        });
    }
    async generate(t, e) {
        let s = {
            ...this.options,
            ...e
        }, { secret: n, crypto: o, base32: r, algorithm: i = "sha1", digits: a = 6 } = s;
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(n), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(o), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireBase32Plugin"])(r);
        let u = e?.guardrails ?? this.guardrails;
        return O({
            secret: n,
            counter: t,
            algorithm: i,
    ...
    async verify(t, e) {
        let s = {
            ...this.options,
            ...e
        }, { secret: n, crypto: o, base32: r, algorithm: i = "sha1", digits: a = 6, counterTolerance: u = 0 } = s;
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(n), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(o), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireBase32Plugin"])(r);
        let p = e?.guardrails ?? this.guardrails;
        return P({
            secret: n,
            token: t.token,
            counter: t.counter,
    ...
    toURI(t = 0) {
        let { issuer: e, label: s, secret: n, algorithm: o = "sha1", digits: r = 6 } = this.options;
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(n), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireLabel"])(s), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireIssuer"])(e), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireBase32String"])(n), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$uri$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateHOTP"])({
            issuer: e,
            label: s,
            secret: n,
            algorithm: o,
...
;
function V(c) {
    let { secret: t, counter: e, algorithm: s = "sha1", digits: n = 6, crypto: o, base32: r, guardrails: i } = c;
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(t), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(o);
    let a = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeSecret"])(t, r);
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateSecret"])(a, i), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateCounter"])(e, i);
    let u = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createCryptoContext"])(o), p = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["counterToBytes"])(e);
    return {
        ctx: u,
        algorithm: s,
        digits: n,
...
async function O(c) {
    let { ctx: t, algorithm: e, digits: s, secretBytes: n, counterBytes: o } = V(c), r = await t.hmac(e, n, o), i = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["dynamicTruncate"])(r);
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["truncateDigits"])(i, s);
}
function E(c) {
    let { ctx: t, algorithm: e, digits: s, secretBytes: n, counterBytes: o } = V(c), r = t.hmacSync(e, n, o), i = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["dynamicTruncate"])(r);
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["truncateDigits"])(i, s);
}
function v(c) {
    let { secret: t, counter: e, token: s, algorithm: n = "sha1", digits: o = 6, crypto: r, base32: i, counterTolerance: a = 0, guardrails: u = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createGuardrails"])() } = c;
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(t), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(r);
    let p = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeSecret"])(t, i);
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateSecret"])(p, u), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateCounter"])(e, u), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateToken"])(s, o), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateCounterTolerance"])(a, u);
    let d = typeof e == "bigint" ? Number(e) : e, R = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeCounterTolerance"])(a).filter((l)=>d + l >= 0);
    return {
        token: s,
        counterNum: d,
        offsets: R,
        crypto: r,
        getGenerateOptions: (l)=>({
                secret: p,
                counter: l,
                algorithm: n,
                digits: o,
...
async function P(c) {
    let { token: t, counterNum: e, offsets: s, crypto: n, getGenerateOptions: o } = v(c);
    for (let r of s){
        let i = e + r, a = await O(o(i));
        if (n.constantTimeEqual(a, t)) return {
            valid: !0,
            delta: r
        };
    }
...
"[project]/node_modules/@otplib/totp/dist/index.js [app-route] (ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@otplib/core/dist/index.js [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$hotp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__ = __turbopack_context__.i("[project]/node_modules/@otplib/hotp/dist/index.js [app-route] (ecmascript) <locals>");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$uri$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@otplib/uri/dist/index.js [app-route] (ecmascript)");
;
;
;
...
var P = class {
    ...
    guardrails;
    constructor(e = {}){
        this.options = e, this.guardrails = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createGuardrails"])(e.guardrails);
    }
    generateSecret() {
        let { crypto: e, base32: r } = this.options;
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(e), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireBase32Plugin"])(r), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateSecret"])({
            crypto: e,
            base32: r
        });
    }
    async generate(e) {
        let r = {
            ...this.options,
            ...e
        }, { secret: i, crypto: o, base32: n, algorithm: c = "sha1", digits: l = 6, period: u = 30, epoch: t, t0: a = 0 } = r;
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(i), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(o), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireBase32Plugin"])(n);
        let p = e?.guardrails ?? this.guardrails;
        return b({
            secret: i,
            algorithm: c,
            digits: l,
            period: u,
            epoch: t ?? Math.floor(Date.now() / 1e3),
            t0: a,
            crypto: o,
            base32: n,
            guardrails: p
    ...
    async verify(e, r) {
        let i = {
            ...this.options,
            ...r
        }, { secret: o, crypto: n, base32: c, algorithm: l = "sha1", digits: u = 6, period: t = 30, epoch: a, t0: p = 0, epochTolerance: T = 0 } = i;
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(o), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(n), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireBase32Plugin"])(c);
        let O = r?.guardrails ?? this.guardrails;
        return C({
            secret: o,
            token: e,
            algorithm: l,
            digits: u,
            period: t,
            epoch: a ?? Math.floor(Date.now() / 1e3),
            t0: p,
            epochTolerance: T,
            crypto: n,
            base32: c,
    ...
    toURI(e) {
        let { issuer: r, label: i, secret: o, algorithm: n = "sha1", digits: c = 6, period: l = 30 } = this.options, u = e?.label ?? i, t = e?.issuer ?? r, a = e?.secret ?? o;
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(a), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireLabel"])(u), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireIssuer"])(t), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireBase32String"])(a), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$uri$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateTOTP"])({
            issuer: t,
            label: u,
            secret: a,
            algorithm: n,
...
;
function S(s) {
    let { secret: e, epoch: r = Math.floor(Date.now() / 1e3), t0: i = 0, period: o = 30, algorithm: n = "sha1", digits: c = 6, crypto: l, base32: u, guardrails: t = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createGuardrails"])() } = s;
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(e), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(l);
    let a = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeSecret"])(e, u);
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateSecret"])(a, t), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateTime"])(r), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validatePeriod"])(o, t);
    let p = Math.floor((r - i) / o);
    return {
        secret: a,
        counter: p,
        algorithm: n,
...
async function b(s) {
    let e = S(s);
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$hotp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["generate"])(e);
}
function K(s) {
    let e = S(s);
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$hotp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["generateSync"])(e);
}
function R(s) {
    let { secret: e, token: r, epoch: i = Math.floor(Date.now() / 1e3), t0: o = 0, period: n = 30, algorithm: c = "sha1", digits: l = 6, crypto: u, base32: t, epochTolerance: a = 0, guardrails: p = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createGuardrails"])() } = s;
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireSecret"])(e), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["requireCryptoPlugin"])(u);
    let T = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeSecret"])(e, t);
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateSecret"])(T, p), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateTime"])(i), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validatePeriod"])(n, p), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateToken"])(r, l), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateEpochTolerance"])(a, n, p);
    let O = Math.floor((i - o) / n), [M, q] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeEpochTolerance"])(a), I = Math.max(0, Math.floor((i - M - o) / n)), w = Math.floor((i + q - o) / n);
    return {
        token: r,
        crypto: u,
        minCounter: I,
...
async function C(s) {
    let { token: e, crypto: r, minCounter: i, maxCounter: o, currentCounter: n, t0: c, period: l, getGenerateOptions: u } = R(s);
    for(let t = i; t <= o; t++){
        let a = await b(u(t));
        if (r.constantTimeEqual(a, e)) return {
            valid: !0,
            delta: t - n,
            epoch: t * l + c
        };
...
function $(s = Math.floor(Date.now() / 1e3), e = 30, r = 0, i = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createGuardrails"])()) {
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateTime"])(s), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validatePeriod"])(e, i), (Math.floor((s - r) / e) + 1) * e + r - s;
}
function ee(s = Math.floor(Date.now() / 1e3), e = 30, r = 0, i = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createGuardrails"])()) {
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validateTime"])(s), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validatePeriod"])(e, i), Math.floor((s - r) / e);
}
;
 //# sourceMappingURL=index.js.map
}),
"[project]/node_modules/@scure/base/index.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
"[project]/node_modules/@otplib/plugin-base32-scure/dist/index.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$scure$2f$base$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@scure/base/index.js [app-route] (ecmascript)");
;
var r = class {
    name = "scure";
    encode(o, e = {}) {
        let { padding: t = !1 } = e, n = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$scure$2f$base$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["base32"].encode(o);
        return t ? n : n.replace(/=+$/, "");
    }
    decode(o) {
        try {
            let e = o.toUpperCase(), t = e.padEnd(Math.ceil(e.length / 8) * 8, "=");
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$scure$2f$base$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["base32"].decode(t);
        } catch (e) {
            throw e instanceof Error ? new Error(`Invalid Base32 string: ${e.message}`) : new Error("Invalid Base32 string");
        }
    }
}, d = Object.freeze(new r), i = r;
;
 //# sourceMappingURL=index.js.map
}),
...
"[project]/node_modules/@noble/hashes/legacy.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class _SHA1 extends __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$_md$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["HashMD"] {
    A = SHA1_IV[0] | 0;
    B = SHA1_IV[1] | 0;
    C = SHA1_IV[2] | 0;
    D = SHA1_IV[3] | 0;
    ...
    process(view, offset) {
        ...
        for(let i = 16; i < 80; i++)SHA1_W[i] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rotl"])(SHA1_W[i - 3] ^ SHA1_W[i - 8] ^ SHA1_W[i - 14] ^ SHA1_W[i - 16], 1);
        // Compression function main loop, 80 rounds
        let { A, B, C, D, E } = this;
        for(let i = 0; i < 80; i++){
            let F, K;
            if (i < 20) {
                F = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$_md$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["Chi"])(B, C, D);
                K = 0x5a827999;
            } else if (i < 40) {
                F = B ^ C ^ D;
                K = 0x6ed9eba1;
            } else if (i < 60) {
                F = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$_md$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["Maj"])(B, C, D);
                K = 0x8f1bbcdc;
            } else {
                F = B ^ C ^ D;
                K = 0xca62c1d6;
            }
            const T = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rotl"])(A, 5) + F + E + K + SHA1_W[i] | 0;
            E = D;
            D = C;
            C = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rotl"])(B, 30);
            B = A;
            A = T;
        }
    ...
    roundClean() {
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["clean"])(SHA1_W);
    }
    destroy() {
        this.set(0, 0, 0, 0, 0);
...
const sha1 = /* @__PURE__ */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createHasher"])(()=>new _SHA1());
...
class _MD5 extends __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$_md$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["HashMD"] {
    A = MD5_IV[0] | 0;
    B = MD5_IV[1] | 0;
    C = MD5_IV[2] | 0;
    D = MD5_IV[3] | 0;
    ...
    process(view, offset) {
        ...
        for(let i = 0; i < 64; i++){
            ...
            if (i < 16) {
                ...
                F = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$_md$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["Chi"])(D, B, C);
                g = (5 * i + 1) % 16;
                s = [
                    5,
                    9,
            ...
            D = C;
            C = B;
            B = B + (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rotl"])(F, s[i % 4]);
        }
        // Add the compressed chunk to the current hash value
        A = A + this.A | 0;
        B = B + this.B | 0;
    ...
    roundClean() {
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["clean"])(MD5_W);
    }
    destroy() {
        this.set(0, 0, 0, 0);
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["clean"])(this.buffer);
...
const md5 = /* @__PURE__ */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createHasher"])(()=>new _MD5());
// RIPEMD-160
...
class _RIPEMD160 extends __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$_md$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["HashMD"] {
    h0 = 0x67452301 | 0;
    h1 = 0xefcdab89 | 0;
    h2 = 0x98badcfe | 0;
    h3 = 0x10325476 | 0;
    ...
    process(view, offset) {
        ...
        for(let group = 0; group < 5; group++){
            ...
            for(let i = 0; i < 16; i++){
                const tl = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rotl"])(al + ripemd_f(group, bl, cl, dl) + BUF_160[rl[i]] + hbl, sl[i]) + el | 0;
                al = el, el = dl, dl = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rotl"])(cl, 10) | 0, cl = bl, bl = tl; // prettier-ignore
            }
            // 2 loops are 10% faster
            for(let i = 0; i < 16; i++){
                const tr = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rotl"])(ar + ripemd_f(rGroup, br, cr, dr) + BUF_160[rr[i]] + hbr, sr[i]) + er | 0;
                ar = er, er = dr, dr = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rotl"])(cr, 10) | 0, cr = br, br = tr; // prettier-ignore
            }
        }
        // Add the compressed chunk to the current hash value
    ...
    roundClean() {
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["clean"])(BUF_160);
    }
    destroy() {
        this.destroyed = true;
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["clean"])(this.buffer);
...
const ripemd160 = /* @__PURE__ */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createHasher"])(()=>new _RIPEMD160()); //# sourceMappingURL=legacy.js.map
}),
...
"[project]/node_modules/@otplib/plugin-crypto-noble/dist/index.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$legacy$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@noble/hashes/legacy.js [app-route] (ecmascript)");
...
var t = class {
    name = "noble";
    hmac(r, n, a) {
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$hmac$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["hmac"])(r === "sha1" ? __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$legacy$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sha1"] : r === "sha256" ? __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$sha2$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sha256"] : __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$sha2$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sha512"], n, a);
    }
    randomBytes(r) {
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$noble$2f$hashes$2f$utils$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["randomBytes"])(r);
    }
    constantTimeEqual(r, n) {
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["constantTimeEqual"])(r, n);
    }
}, A = Object.freeze(new t), b = t;
;
 //# sourceMappingURL=index.js.map
}),
"[project]/node_modules/otplib/dist/index.js [app-route] (ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@otplib/core/dist/index.js [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$hotp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__ = __turbopack_context__.i("[project]/node_modules/@otplib/hotp/dist/index.js [app-route] (ecmascript) <locals>");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$totp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__ = __turbopack_context__.i("[project]/node_modules/@otplib/totp/dist/index.js [app-route] (ecmascript) <locals>");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$uri$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@otplib/uri/dist/index.js [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$plugin$2d$base32$2d$scure$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@otplib/plugin-base32-scure/dist/index.js [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$plugin$2d$crypto$2d$noble$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@otplib/plugin-crypto-noble/dist/index.js [app-route] (ecmascript)");
;
;
;
;
...
;
;
var c = Object.freeze(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$plugin$2d$crypto$2d$noble$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NobleCryptoPlugin"]), g = Object.freeze(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$plugin$2d$base32$2d$scure$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["ScureBase32Plugin"]);
function l(t) {
    return {
        secret: t.secret,
        ...
        t0: t.t0 ?? 0,
        counter: t.counter,
        guardrails: t.guardrails ?? (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createGuardrails"])()
    };
}
function f(t) {
    return {
...
function u(t, e, r) {
    ...
    if (t === "hotp") {
        if (e === void 0) throw new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["ConfigurationError"]("Counter is required for HOTP strategy. Example: { strategy: 'hotp', counter: 0 }");
        return r.hotp(e);
    }
    throw new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["ConfigurationError"](`Unknown OTP strategy: ${t}. Valid strategies are 'totp' or 'hotp'.`);
}
function j(t) {
    let { crypto: e = c, base32: r = g, length: o = 20 } = t || {};
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$core$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateSecret"])({
        crypto: e,
        base32: r,
        length: o
    });
...
function O(t) {
    let { strategy: e = "totp", issuer: r, label: o, secret: i, algorithm: a = "sha1", digits: p = 6, period: n = 30, counter: s } = t;
    return u(e, s, {
        totp: ()=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$uri$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateTOTP"])({
                issuer: r,
                label: o,
                secret: i,
                algorithm: a,
        ...
        hotp: (y)=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$uri$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateHOTP"])({
                issuer: r,
                label: o,
                secret: i,
                algorithm: a,
...
async function m(t) {
    let e = l(t), { secret: r, crypto: o, base32: i, algorithm: a, digits: p } = e, n = {
        secret: r,
        crypto: o,
        base32: i,
        algorithm: a,
    ...
    return u(e.strategy, e.counter, {
        totp: ()=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$totp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["generate"])({
                ...n,
                period: e.period,
                epoch: e.epoch,
                t0: e.t0,
        ...
        hotp: (s)=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$hotp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["generate"])({
                ...n,
                counter: s,
                guardrails: e.guardrails
            })
...
function T(t) {
    ...
    return u(e.strategy, e.counter, {
        totp: ()=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$totp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["generateSync"])({
                ...n,
                period: e.period,
                epoch: e.epoch,
                t0: e.t0,
        ...
        hotp: (s)=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$hotp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["generateSync"])({
                ...n,
                counter: s,
                guardrails: e.guardrails
            })
...
async function P(t) {
    let e = f(t), { secret: r, token: o, crypto: i, base32: a, algorithm: p, digits: n } = e, s = {
        secret: r,
        token: o,
        crypto: i,
        base32: a,
    ...
    return u(e.strategy, e.counter, {
        totp: ()=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$totp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["verify"])({
                ...s,
                period: e.period,
                epoch: e.epoch,
                t0: e.t0,
        ...
        hotp: (y)=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$hotp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["verify"])({
                ...s,
                counter: y,
...
function h(t) {
    ...
    return u(e.strategy, e.counter, {
        totp: ()=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$totp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["verifySync"])({
                ...s,
                period: e.period,
                epoch: e.epoch,
                t0: e.t0,
        ...
        hotp: (y)=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$otplib$2f$hotp$2f$dist$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["verifySync"])({
                ...s,
                counter: y,
                counterTolerance: e.counterTolerance,
                guardrails: e.guardrails
...
var d = class {
    ...
    base32;
    constructor(e = {}){
        let { strategy: r = "totp", crypto: o = c, base32: i = g } = e;
        this.strategy = r, this.crypto = o, this.base32 = i;
    }
    getStrategy() {
...
"[project]/node_modules/next/dist/esm/build/templates/app-route.js { INNER_APP_ROUTE => \"[project]/app/sys-api/auth/2fa/verify/route.ts [app-route] (ecmascript)\" } [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$esm$2f$server$2f$response$2d$cache$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__ = __turbopack_context__.i("[project]/node_modules/next/dist/esm/server/response-cache/index.js [app-route] (ecmascript) <locals>");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$esm$2f$server$2f$response$2d$cache$2f$types$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next/dist/esm/server/response-cache/types.js [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$sys$2d$api$2f$auth$2f$2fa$2f$verify$2f$route$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/app/sys-api/auth/2fa/verify/route.ts [app-route] (ecmascript)");
;
;
;
;
...
const routeModule = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$esm$2f$server$2f$route$2d$modules$2f$app$2d$route$2f$module$2e$compiled$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["AppRouteRouteModule"]({
    definition: {
        ...
        page: "/sys-api/auth/2fa/verify/route",
        pathname: "/sys-api/auth/2fa/verify",
        filename: "route",
        bundlePath: ""
    },
    distDir: ("TURBOPACK compile-time value", ".next\\dev") || '',
    relativeProjectDir: ("TURBOPACK compile-time value", "") || '',
    resolvedPagePath: "[project]/app/sys-api/auth/2fa/verify/route.ts",
    nextConfigOutput,
    userland: __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$sys$2d$api$2f$auth$2f$2fa$2f$verify$2f$route$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__
});
...
async function handler(req, res, ctx) {
    ...
    let srcPage = "/sys-api/auth/2fa/verify/route";
...
//# sourceMappingURL=node_modules_145ebe51._.js.map