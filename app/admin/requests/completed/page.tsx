...
export default function CompletedRequestsPage() {
    ...
    const handleCreate = async (data: Partial<ServiceRequest>) => {
        try {
            const requestData = {
                ...
                status: "COMPLETED", // Default to completed for this page
            }
    ...
    const handleExport = () => {
        try {
            ...
            link.href = url
            link.setAttribute('download', `completed_requests_${format(new Date(), 'yyyy-MM-dd')}.csv`)
                    ...
                    <div className="p-8">
                        {searchActive || filtersActive ? (
                            <EmptyState
                                ...
                                description={
                                    searchActive
                                        ? `No completed requests found matching "${searchQuery}"`
                                        : "No completed requests match the current filters"
                                }
                                action={
                                    <Button
                                        variant="outline"
                        ...
                        <div className="p-4 border-b bg-muted/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div className="text-sm text-muted-foreground">
                                {searchActive || filtersActive ? (
                                    ...
                                    <>
                                        Showing all <span className="font-semibold">{shownRequests}</span> completed request{shownRequests !== 1 ? 's' : ''}
                                    </>
                                )}
                            </div>
