...
import { getRiderById, updateRider, type Rider } from "@/lib/api"
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">{rider?.external_id}</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded p-6 shadow-sm max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="p-3 bg-blue-50 rounded border border-blue-200">
            <h3 className="text-sm font-medium text-blue-800 mb-1">Rider Information</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-gray-600">Rider ID:</span>
                <span className="ml-2 font-mono">{rider?.external_id}</span>
              </div>
              <div>
                <span className="text-gray-600">Role:</span>
                <span className="ml-2">{rider?.role}</span>
              </div>
              <div>
                <span className="text-gray-600">Created:</span>
                <span className="ml-2">{rider?.created_at ? new Date(rider.created_at).toLocaleDateString() : "-"}</span>
              </div>
              <div>
                <span className="text-gray-600">Last Updated:</span>
                <span className="ml-2">{rider?.updated_at ? new Date(rider.updated_at).toLocaleDateString() : "-"}</span>
              </div>
            </div>
          </div>
